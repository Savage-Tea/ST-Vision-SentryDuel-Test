...
| ① | 简单搜索 + policy model | 可跑可上传；整体胜率约 50%，颜色不对称待解 |
| ② | MCTS / PUCT | **已实现，但当前打不过 phase①**（见下） |
| ③ | 自对弈 + 网络（α-zero 式策略迭代） | 待训练，需算力 |

## 阶段② 现状：MCTS 目前不如穷举搜索

`brain/mcts.{h,cpp}` 已实现：PUCT 选择、一步估值做 softmax 先验、
负极大值回传、时间预算、节点上限，以及自对弈需要的 Dirichlet 噪声与
按访问分布采样。

**实测（vs 确定性对手 det_ai_a）**

| 策略 | 执红 | 执蓝 |
|---|---|---|
| phase① 本回合穷举 | **21:10 胜** | **26:3 胜** |
| MCTS | 10:13 负 | 10:17 负 |

这与开工前的判断一致：本游戏**完全确定性、分支因子仅 ~7**，属于
"穷举搜索比采样更省样本"的形态。MCTS 的优势要等到先验与价值由网络
给出之后才会体现——那正是阶段③ 要做的事，届时 MCTS 是策略改进算子，
不必重写。

已修的坑：负极大值回传的前提是父子节点**轮流走子**；而一个行动阶段内
最多 3 步，同阶段父子走子方相同，此时取负会让值在阶段内翻符号，根节点
会认为"什么都不做"最优。修复后 0:15 → 10:13。

开关：`ST_MCTS=1` 走 MCTS，默认走 phase①，便于同口径 A/B。
