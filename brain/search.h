...
// 值网络混合系数 β：叶值 = (1-β)·evaluate() + β·V。β=0 纯手写，β=1 纯网络。
//
// 存在的理由：**argmax 对噪声是有偏的**。手写评估在"同一回合内不同候选
// 之间"的差异是系统性的；而值网络是在跨状态的结局上训练的，它在回合内的
// 取值差异很可能被自身噪声淹没 —— 此时从几百个叶子里硬取最大，等于挑噪声。
// 实测：纯网络（β=1）池平均 0.0989，而手写是 0.947。所以要么别用网络，
// 要么用混合把它的贡献压在噪声之下。
